-- // FLUXHUB | Sheriff vs Murders Duels v2.3 (FIXED AIM & HITBOX)
...
-- Функция поиска папки врага (универсальная)
...
-- ============================================================
-- // РАЗДЕЛ "SILENT AIM" (ПОЛНОСТЬЮ ПЕРЕРАБОТАН)
-- ============================================================
...
-- Функция поиска ближайшего врага (использует атрибуты игры)
local function getClosestPlayer()
    ...
    for _, plr in pairs(players:GetPlayers()) do
        if plr ~= player then
            ...
            -- Проверка на принадлежность к той же игре и противоположной команде
...
-- Основной метод Silent Aim через hookmetamethod
local oldIndex
...
-- Кнопки управления Silent Aim
AimbotTab:CreateToggle({
    Name = "Silent Aim (Auto Aim on Click)",
    CurrentValue = false,
    Callback = function(Value)
        SilentAimEnabled = Value
        -- Включаем/выключаем перехват
        if Value then
            -- Включаем режим (уже активен через hook)
            print("Silent Aim enabled")
        else
            -- Отключаем (перезаписываем хук, чтобы он ничего не делал)
            oldIndex = hookmetamethod(game, "__index", function(self, key)
                if not checkcaller() and self == player:GetMouse() and key == "Hit" then
                    return oldIndex(self, key)
                end
...
-- // ИНФОРМАЦИЯ
-- ============================================================
print("✅ FLUXHUB v2.3 | Sheriff vs Murders Duels loaded (FIXED)!")
print("🔥 Silent Aim (hook), Hitbox Expander, Spinbot, Kill Aura, BHop, Model Changer")
